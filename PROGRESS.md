...
| R2 | Refresh redesign review | ✅ Done | Opus review: Fixed bug in SessionManager.subscribe() auto-resume — was calling session.refresh() (requires RUNNING) on STOPPED sessions, now calls session.start() directly. Removed dead `binary` variable in _buildResumeCommand. Verified: no SHELL/respawn-pane/_tmuxRunningClaude/session:restart remnants; TmuxSession alive path is kill-PTY+re-attach only; dead path resets to STOPPED then start(); DirectSession.refresh() correctly guards RUNNING-only; session:subscribed sent after subscribe (TerminalPane handles xterm.reset); session:refreshed updates client state; SessionCard/SessionOverlay fully clean of shell status. 440 tests passing. |
