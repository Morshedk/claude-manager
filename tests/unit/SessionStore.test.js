...
describe('SessionStore - saveSnapshot() / loadSnapshot() round-trip', () => {
  test('saves and loads a snapshot string round-trip', async () => {
    const snapshotData = '\x1b[?1049h\x1b[H$ hello world\r\n';
    await store.saveSnapshot('sess-xyz', snapshotData);
    const loaded = await store.loadSnapshot('sess-xyz');
    expect(loaded).toBe(snapshotData);
  });

  test('returns empty string for missing snapshot', async () => {
    const result = await store.loadSnapshot('nonexistent-id');
    expect(result).toBe('');
  });

  test('overwrites an existing snapshot', async () => {
    await store.saveSnapshot('sess-1', 'old snapshot');
    await store.saveSnapshot('sess-1', 'new snapshot');
    const loaded = await store.loadSnapshot('sess-1');
    expect(loaded).toBe('new snapshot');
  });

  test('creates the snapshots directory if it does not exist', async () => {
    // tmpDir has no snapshots/ subdirectory yet
    expect(fs.existsSync(store.snapshotsDir)).toBe(false);
    await store.saveSnapshot('sess-new', 'data');
    expect(fs.existsSync(store.snapshotsDir)).toBe(true);
    expect(await store.loadSnapshot('sess-new')).toBe('data');
  });
});

// ========== deleteSnapshot() ==========

describe('SessionStore - deleteSnapshot()', () => {
  test('removes the snapshot file', async () => {
    await store.saveSnapshot('to-delete', 'some data');
    await store.deleteSnapshot('to-delete');
    const result = await store.loadSnapshot('to-delete');
    expect(result).toBe('');
  });

  test('is a no-op when snapshot does not exist', async () => {
    // Should not throw
    await expect(store.deleteSnapshot('does-not-exist')).resolves.toBeUndefined();
  });
});
