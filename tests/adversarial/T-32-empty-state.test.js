...
const PORTS = { direct: 3129, tmux: 3729 };
const MODES = ['direct', 'tmux'];

for (const MODE of MODES) {
const PORT = PORTS[MODE];
const BASE_URL = `http://127.0.0.1:${PORT}`;
const SCREENSHOTS_DIR = path.join(APP_DIR, 'qa-screenshots', `T-32-empty-state-${MODE}`);
...
test.describe(`T-32 — Session list empty state message [${MODE}]`, () => {
  let serverProc = null;
  let tmpDir = '';
  let projPath = '';
