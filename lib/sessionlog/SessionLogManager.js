...
function isSpinnerLine(t) {
  ...
  // Unicode-spinner-prefixed partial animation words: Claude Code spells out "Undulating…"
  // one character at a time using in-place updates. After ANSI stripping these appear as
  // lines like "✢ Un ul", "✶    ul ti", "·     lating" — growing partial words.
  // Unicode spinner chars (✻✽✢·✶○◆▶➜) are EXCLUSIVELY used by Claude Code's animation.
  // Threshold 14 covers the longest animation word ("Undulating…" = 11 chars + prefix).
  if (nonWs.length <= 14 && /^[✻✽✢·✶○◆▶➜]/.test(t)) return true;
  // ASCII * spinner fragment. Threshold matches Unicode spinners — in session-log context
  // all legitimate * lines (git branch, npm) arrive with ⎿ prefix, never bare at col 0.
  if (nonWs.length <= 14 && /^\*/.test(t)) return true;
  // Full spinner status line: "· Undulating… (25m 32s · ↓ 72.1k tokens · thought for 43s)"
  // These start with a spinner char and contain timing/token info.
  if (/\btokens\b/.test(t) && /^[✻✽✢·✶○◆▶➜*\s]/.test(t)) return true;
