...
export function createRouter(deps = {}) {
  const router = Router();
  const { sessions, projects, settings, terminals, detector, todos, watchdog, sessionLog, dataDir, health } = deps;
  ...
  /** POST /api/sessions/recover — scan tmux for orphan sessions and adopt them */
  router.post('/api/sessions/recover', async (req, res) => {
    if (!sessions) return res.status(503).json({ error: 'Sessions not available' });
    try {
      const result = await sessions.recoverOrphans();
      res.json(result);
    } catch (e) {
  ...
  router.get('/api/audit/digests', (req, res) => {
    ...
    try {
      ...
      const dir = projectPath
        ...
        : path.join(DATA_DIR, 'audit', 'daily');
      if (!fs.existsSync(dir)) return res.json([]);
  ...
  router.get('/api/audit/reviews', (req, res) => {
    ...
    try {
      ...
      const dir = projectPath
        ...
        : path.join(DATA_DIR, 'audit', 'reviews');
      if (!fs.existsSync(dir)) return res.json([]);
      const files = fs.readdirSync(dir)
        .filter(f => f.endsWith('.md'))
        .sort()
  ...
  router.get('/api/pm/reports', (req, res) => {
    if (!watchdog) return res.json([]);
    try {
      const dir = path.join(DATA_DIR, 'pm', 'daily');
      if (!fs.existsSync(dir)) return res.json([]);
      const files = fs.readdirSync(dir)
        .filter(f => f.endsWith('.md'))
        .sort()
  ...
  /** POST /api/audit/trigger — manually trigger a daily audit */
  router.post('/api/audit/trigger', (req, res) => {
    if (!watchdog) return res.status(503).json({ error: 'watchdog not available' });
    if (!watchdog.auditDaily) return res.status(501).json({ error: 'auditDaily not available' });
    res.json({ status: 'started', message: 'Daily audit triggered — runs in background' });
    watchdog.auditDaily().catch(() => {});
  });

  /** POST /api/audit/trigger-weekly — manually trigger a weekly review */
  router.post('/api/audit/trigger-weekly', (req, res) => {
    if (!watchdog) return res.status(503).json({ error: 'watchdog not available' });
    if (!watchdog.auditWeeklyReview) return res.status(501).json({ error: 'auditWeeklyReview not available' });
    res.json({ status: 'started', message: 'Weekly review triggered — runs in background' });
    watchdog.auditWeeklyReview().catch(() => {});
  });

  /** POST /api/pm/trigger — manually trigger a PM cycle */
  router.post('/api/pm/trigger', (req, res) => {
    if (!watchdog) return res.status(503).json({ error: 'watchdog not available' });
    if (!watchdog.pmCycle) return res.status(501).json({ error: 'pmCycle not available' });
    res.json({ status: 'started', message: 'PM cycle triggered — runs in background' });
    watchdog.pmCycle().catch(() => {});
  });
  ...
  /** GET /api/health — return current health state directly from in-memory module */
  router.get('/api/health', (req, res) => {
    try {
      if (health && typeof health.get === 'function') return res.json(health.get());
      // Fallback: read from file if health module not injected
