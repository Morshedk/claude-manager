...
// Load governance framework reference at module init
let FRAMEWORKS_REF = '';
try {
  FRAMEWORKS_REF = fs.readFileSync(path.join(__dirname, 'audit-frameworks.md'), 'utf8');
} catch {
  log.warn(TAG, 'audit-frameworks.md not found — auditor will run without reference framework');
}
...
+ (FRAMEWORKS_REF ? `\n\n## Reference Framework\n\nUse the following governance framework to ground your evaluation. Check applicable items and reference framework sections when issuing recommendations.\n\n${FRAMEWORKS_REF}` : '');
...
+ (FRAMEWORKS_REF ? `\n\n## Reference Framework\n\nUse the governance framework below to inform your scoring, pattern analysis, and strategic assessment.\n\n${FRAMEWORKS_REF}` : '');
