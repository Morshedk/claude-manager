...
const TMUX = (() => { try { return execSync('which tmux', { encoding: 'utf8' }).trim(); } catch { return '/usr/bin/tmux'; } })();
...
{"nextStep": "one specific actionable task", "why": "one sentence reason"}`;
  ...
  async tick() {
    ...
    try {
      ...
      this.emit('tick', { count: this._tickCount, creditState: this._creditState, lowPower: this._lowPower, vitals: this._lastVitals || null });
    } catch (err) {
      this.emit('error', err);
    } finally {
      this._ticking = false;
