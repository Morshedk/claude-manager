...
{"summary": "1-2 sentence summary of what happened", "keyActions": ["action1", "action2"], "state": "productive|error_loop|idle|slow_progress"}
...
export class WatchdogManager extends EventEmitter {
  ...
  _startSession(sess) {
    ...
    const cmd = `env ${envParts.join(' ')} ${CLAUDE} --dangerously-skip-permissions --verbose${channelsFlag}`;
  ...
  async summarizeSession(sessName) {
    ...
    if (match) {
      // v2 SessionManager uses getScrollback(id) which is an alias for getSnapshot
      const sb = this.sessionManager.getScrollback
        ? this.sessionManager.getScrollback(match.id)
        : null;
      // getScrollback may return a Promise in v2
  ...
  _appendLog(sessName, entry) {
    ...
    fs.writeFileSync(this._logFile(sessName), JSON.stringify(trimmed, null, 2));
  }
  ...
  _saveLearnings(entries) {
    ...
    fs.writeFileSync(this._learningsFile, JSON.stringify(trimmed, null, 2));
  }
