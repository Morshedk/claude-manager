...
export class SessionHandlers {
   ...
   * Protocol order: snapshot → subscribed → live output (via viewer callback).
  ...
  async subscribe(clientId, msg) {
    ...
    try {
      ...
      // subscribe() also returns the current snapshot.
      ...
      // Send snapshot first — clean rendered state, no byte-replay artifacts
      if (snapshot) {
  ...
  /**
   * session:restart — restart a stopped/errored session.
   * @param {string} clientId
   * @param {{ type: string, id: string, cols?: number, rows?: number }} msg
   */
