...
export class SessionHandlers {
  ...
  async refresh(clientId, msg) {
    try {
      ...
      // Re-send scrollback immediately after the reset so all viewers recover their
      // terminal content deterministically. Without this, on Mac hardware the PTY
      // replay races xterm.reset(): data arrives first, reset fires, terminal blanks.
      const activeSession = this.sessions.get(msg.id);
      const scrollback = activeSession?.getScrollback ? activeSession.getScrollback() : '';
      if (scrollback) {
        this.registry.broadcastToSession(msg.id, { type: 'session:output', id: msg.id, data: scrollback });
      }
