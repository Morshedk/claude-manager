...
export class SessionHandlers {
  ...
  async subscribe(clientId, msg) {
    ...
    try {
      ...
      console.log(`[sub] id=${id.slice(0,8)} client wants cols=${msg.cols} rows=${msg.rows} | PTY currently cols=${session?.cols} rows=${session?.rows}`);

      // Register viewer — captures scrollback at CURRENT PTY dimensions.
      const scrollback = await this.sessions.subscribe(id, clientId, (data) => {
        this.registry.send(clientId, { type: 'session:output', id, data });
      });
      console.log(`[sub] scrollback len=${scrollback?.length ?? 0} captured at PTY cols=${session?.cols} | client wants cols=${msg.cols}`);

      // Resize PTY to actual terminal dimensions if provided
      if (msg.cols && msg.rows) {
        this.sessions.resize(id, msg.cols, msg.rows);
        console.log(`[sub] resized PTY → cols=${msg.cols} rows=${msg.rows}`);
      }
