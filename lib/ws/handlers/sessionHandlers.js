...
export class SessionHandlers {
  ...
  async subscribe(clientId, msg) {
    ...
    try {
      // Resize PTY FIRST so getScrollback() captures content at the correct
      // terminal dimensions. Scrollback captured before resize reflects old col
      // width and displays incorrectly in the newly-sized terminal.
      if (msg.cols && msg.rows) {
        this.sessions.resize(id, msg.cols, msg.rows);
      }

      // Register viewer — callback streams live PTY output to this client.
      // getScrollback() is called here, after the resize above.
