...
export class DirectSession extends EventEmitter {
  ...
  async refresh(authEnv = {}) {
    ...
    const binary = (this.meta.command || 'claude').split(/\s+/)[0] || 'claude';
    const isClaude = /claude/i.test(binary);

    // For Claude sessions: verify conversation file exists before killing anything.
    // Non-Claude commands (bash, etc.) don't create conversation files — skip check.
    if (isClaude && this.meta.claudeSessionId && !this._claudeSessionFileExists(this.meta.claudeSessionId)) {
      throw new Error(
    ...
    // Re-start (Claude sessions use --resume; non-Claude restarts fresh)
