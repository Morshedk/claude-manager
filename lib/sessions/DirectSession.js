...
export class DirectSession extends EventEmitter {
  ...
  async refresh(authEnv = {}) {
    ...
    // For Claude sessions that are RUNNING: verify conversation file exists before
    // killing anything. From STOPPED/ERROR the PTY is already gone — restart safely.
    if (
      this.meta.status === STATES.RUNNING &&
      isClaude &&
      this.meta.claudeSessionId &&
      !this._claudeSessionFileExists(this.meta.claudeSessionId)
    ) {
      throw new Error(
