...
export class TmuxSession extends EventEmitter {
  ...
  constructor(meta, store, { cols = 120, rows = 30, sessionLogManager = null } = {}) {
    ...
    this.meta = {
      ...
      tmuxName: meta.tmuxName || TmuxSession._buildTmuxName(meta.id, meta.name),
    };
    this.store = store;
    this.pty = null;
    ...
    this._cols = cols;
    this._rows = rows;
    // Rolling ring buffer: accumulates live PTY output for scrollback replay.
    // Populated from _wirePty onData; returned by getScrollback() so new
    // subscribers receive content immediately without a PTY re-attach.
    this._previewBuffer = meta.lastScrollback || '';
  ...
  static _buildTmuxName(id, name) {
    const base = `cm-${id.slice(0, 8)}`;
    if (!name) return base;
    const slug = name.trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 24);
  ...
  /**
   * Return a self-contained snapshot of the tmux pane for replay to a new subscriber.
   * Uses tmux capture-pane so the output is stateless — no prior terminal state needed.
   * Falls back to the ring buffer if the tmux session is gone.
   * @returns {string}
   */
  getScrollback() {
    if (this._tmuxSessionAlive()) {
      try {
        // resize-window before capture-pane: TmuxSession.resize() sends SIGWINCH
        // via node-pty, but tmux processes it asynchronously. If capture-pane
        // runs before tmux handles the SIGWINCH, scrollback is captured at the
        // old (wrong) window width and garbles the terminal display.
        // resize-window goes through the tmux server socket synchronously, so
        // the window is guaranteed to be at the correct dimensions before capture.
        if (this._cols && this._rows) {
          execSync(
            `tmux resize-window -t ${JSON.stringify(this.meta.tmuxName)} -x ${this._cols} -y ${this._rows}`,
            { timeout: 2000 }
          );
        }
        const raw = execSync(
          `tmux capture-pane -t ${JSON.stringify(this.meta.tmuxName)} -p -S -500`,
          { timeout: 2000, encoding: 'utf8' }
        );
        // tmux capture-pane uses \n (LF only) as line separator. xterm.js treats
        // bare \n as "cursor down" without resetting to column 0 (VT100 LF behaviour).
        // The bash prompt on the previous visible line can leave the cursor at column
        // N, causing all subsequent lines to start at column N instead of 0, which
        // splits content lines and garbles the display.
        // Fix: convert \n → \r\n so each capture-pane row starts at column 0 in xterm.
        return raw.replace(/\n/g, '\r\n');
      } catch { /* fall through */ }
    }
  ...
  async refresh(claudeCmd, authEnv = {}, { cols, rows } = {}) {
    ...
    if (this._tmuxSessionAlive()) {
      ...
      this._attachPty();
      if (!this.pty) {
        // PTY failed to attach (e.g. node-pty native build error on Mac).
        // Surface the error so the client receives session:error rather than
        // staying silently RUNNING with all keystrokes dropped.
        this._setState(STATES.ERROR);
        await this.store.upsert(this.meta);
        throw new Error(`PTY failed to attach for session ${this.meta.id} — check node-pty compatibility`);
      }
      // Ensure status is RUNNING — tmux is alive and PTY is reattached.
      // Bypass normal state machine: refresh is a recovery path, not a
      // lifecycle transition.  Without this, a STOPPED session stays
      // STOPPED and write() silently drops all keystrokes.
      if (this.meta.status !== STATES.RUNNING) {
        this.meta.status = STATES.RUNNING;
        this.meta.lastActiveAt = new Date().toISOString();
      }
  ...
  _attachPty() {
    ...
    try {
      ...
      console.error(`[TmuxSession] PTY spawn failed for ${this.meta.id}:`, e.message);
      return;
    }
  ...
  _wirePty() {
    ...
    this.pty.onData((data) => {
      // Accumulate into preview buffer for scrollback replay to new subscribers
  ...
  _setState(newState) {
    ...
    if (newState === STATES.RUNNING || newState === STATES.STARTING) {
      this.meta.lastActiveAt = new Date().toISOString();
    }
   ...
   */
  toJSON() {
    return { ...this.meta, lastScrollback: this._previewBuffer };
  }
}
