...
export class SessionManager extends EventEmitter {
  ...
  /**
   * Scan tmux for cm-* sessions not tracked in memory and adopt them.
   * Also re-attaches sessions that are tracked but marked stopped while tmux is alive.
   * @returns {Promise<{ adopted: string[], reattached: string[] }>}
   */
  async recoverOrphans() {
    ...
    // 1. Find all live cm-* tmux sessions
    let liveTmux = [];
    try {
      const out = execSync('tmux list-sessions -F "#{session_name}"', { timeout: 3000 }).toString().trim();
      liveTmux = out.split('\n').filter(n => n.startsWith('cm-'));
    } catch { return { adopted, reattached, skipped }; }

    // Build lookup of tmuxName → session for tracked sessions
    const trackedByTmux = new Map();
    for (const [, session] of this.sessions) {
      if (session.meta.tmuxName) trackedByTmux.set(session.meta.tmuxName, session);
    }

    for (const tmuxName of liveTmux) {
      const existing = trackedByTmux.get(tmuxName);

      if (existing) {
        // Tracked but stopped — try to re-attach
        if (existing.meta.status === STATES.STOPPED || existing.meta.status === STATES.ERROR) {
          try {
            existing._attachPty();
            if (existing.pty) {
              existing.meta.status = STATES.RUNNING;
              reattached.push(`${tmuxName} (${existing.meta.name || existing.meta.id})`);
              log.info('sessions', 'reattached orphan', { tmuxName, id: existing.meta.id });
            }
          } catch { /* leave as-is */ }
        }
        continue;
      ...
      // Not tracked at all — only adopt if this server created it
      const idPrefix = tmuxName.replace('cm-', '');
      const id = `${idPrefix}-${uuidv4().slice(9)}`;

      const lifecycleMeta = this._findLifecycleEntry(tmuxName);

      // Skip sessions not in this server's lifecycle log — they belong
      // to the other server (prod vs beta) or were created externally.
      if (!lifecycleMeta) {
        skipped.push(tmuxName);
        continue;
      }

      // Detect cwd from the process running inside tmux
      let cwd = null;
      try {
        const panePid = execSync(`tmux list-panes -t ${JSON.stringify(tmuxName)} -F "#{pane_pid}"`, { timeout: 2000 }).toString().trim();
        const childPid = execSync(`pgrep -P ${panePid} | head -1`, { timeout: 2000 }).toString().trim();
        if (childPid) cwd = execSync(`readlink -f /proc/${childPid}/cwd`, { timeout: 2000 }).toString().trim();
      } catch { /* use fallback */ }

      // Match to a project by cwd
      let projectId = null;
      let projectName = 'Unknown';
      if (cwd) {
        const allProjects = this.projectStore.list?.() || [];
        const match = allProjects.find(p => p.path === cwd);
        if (match) { projectId = match.id; projectName = match.name; }
      }

      const meta = {
        id: lifecycleMeta?.id || id,
        projectId: projectId || lifecycleMeta?.projectId || null,
        projectName: projectName !== 'Unknown' ? projectName : (lifecycleMeta?.projectName || projectName),
        name: lifecycleMeta?.name || tmuxName,
        command: lifecycleMeta?.command || 'claude',
        cwd: cwd || lifecycleMeta?.cwd || process.cwd(),
        mode: 'tmux',
        claudeSessionId: lifecycleMeta?.claudeSessionId || null,
        tmuxName,
        telegramEnabled: lifecycleMeta?.telegramEnabled || false,
        startedAt: new Date().toISOString(),
        status: STATES.CREATED,
      };

      const session = new TmuxSession(meta, this.store, { sessionLogManager: this.sessionLogManager });
      session.meta = { ...session.meta, ...meta, tmuxName };
      this._wireSessionEvents(session);
      this.sessions.set(meta.id, session);
      ...
      adopted.push(`${tmuxName} → "${meta.name}" (${session.meta.status})`);
      log.info('sessions', 'adopted orphan tmux session', { tmuxName, id: meta.id, name: meta.name });
    }

    if (adopted.length || reattached.length) {
      await this.store.save(this.sessions).catch(() => {});
      this.emit('sessionsChanged');
    }

    return { adopted, reattached, skipped };
  }

  /**
   * Search session-lifecycle.json for the original metadata of a tmux session.
   * @param {string} tmuxName
   * @returns {object|null}
   */
  _findLifecycleEntry(tmuxName) {
    try {
      const lifecyclePath = join(this.store.dataDir, 'session-lifecycle.json');
      if (!existsSync(lifecyclePath)) return null;
      const entries = JSON.parse(readFileSync(lifecyclePath, 'utf8'));
      if (!Array.isArray(entries)) return null;
      // Walk backwards to find the most recent 'created' event for this tmux name
      const idPrefix = tmuxName.replace('cm-', '');
      for (let i = entries.length - 1; i >= 0; i--) {
        const e = entries[i];
        if (e.event === 'created' && e.sessionId?.startsWith(idPrefix)) {
          return {
            id: e.sessionId,
            name: e.name,
  ...
  async subscribe(sessionId, clientId, callback) {
    ...
    if (
      ...
      try {
        ...
        log.warn('sessions', 'auto-resume failed', { sessionId, error: err.message });
      }
    }
  ...
  startPreviewInterval(settingsStore, broadcastFn) {
    ...
    this._previewInterval = setInterval(async () => {
      ...
      for (const session of this.sessions.values()) {
        ...
        try {
          // Prefer clean log data (filtered, includes lifecycle events); fall back to raw PTY
          let lines;
  ...
  _wireSessionEvents(session) {
    session.on('stateChange', (e) => {
      ...
      if (e.state === 'running') {
        ...
        log.info('sessions', 'session stopped', { id: session.meta.id });
